...
layout: post
title: VAE
date: 2026-01-22 12:50
categories: MyStudy 
tags: DeepLearning VAE
math: true
---
...
KL term이 $q_\phi(z\mid x)$를 ${N}(0,I)$ 근처로 끌어당기면서 데이터 전체를 하나의 **연속적인 공통 공간**에 들어가게 만든다. 

![alt](/assets/img/interpolate.gif)

그 결과 latent interpolation이 꽤 자연스럽다
> 공통 공간에 잇으니까 두 점 사이를 선형 보간해도 자연스러운 중간 형태가 나올 가능성이 높다

  학습 후 $z\sim{N}(0,I)$에서 샘플링하면 고확률 영역에서 뽑으면 그럴듯한 샘플이 나오는 경향이 있다.

prior가 multimodal이나 이상하면 어디에서 샘플링해야 안전한지 latent 거리가 의미 있는지 등이 훨씬 애매해진다. 
>prior가 multimodal  
![alt](/assets/img/vae.nulti.png)
prior 분포 p(z)가 여러 개의 피크를 가진 분포
{: .prompt-info }
...
표준 정규 prior는 **너무 단순해서 over-regularization**을 일으키고 latent가 충분히 정보를 못 담는 문제가 있다.
>KL term이 너무 강력해서 encoder가 N(0,1)을 지나치게 따르려고 해서 reconstruction성능이 떨어지는 문제

최적 prior는 aggregated posterior $p(z)=\frac{1}{N}\sum_x q_\phi(z\mid x)$ 인데, 이건 계산이 불가능해서 VampPrior 같은 근사 기법이 연구되고 있다. 


>aggregated posterior  
각 데이터 x^(i)에 대해 encoder가 만든 분포:  
$q_\phi(z|x¹), q_\phi(z|x²), ..., q_\phi(z|xᴺ)$  
Aggregated posterior:  
$p(z) = \frac{1}{N} Σᵢ q_\phi(z|xⁱ)$  ← **실제 데이터 분포를 반영한 최적 prior**  
이론적으로 이걸 prior로 쓰면 VAE 완벽!
{: .prompt-info }

>왜 계산 불가능?  
    1. N=수백만 → 계산량 폭발  
    2. encoder 파라미터 $\phi$ 에 의존 → 학습하면서 계속 바뀜  
    3. closed-form 없음 → 수치적근사도 불가능  

>VampPrior  
전체 데이터 N개 다 못 써도 대표 샘플 K개로 aggregated posterior 근사  
    1. 데이터 일부 뽑아서 "대표 샘플" K개 선택  
    2. 이 K개로 aggregated posterior 근사  
   $p(z) ≈ \frac{1}{K} Σₖ q_\phi(z|xₖ)$  
    3. $KL(q_\phi(z|x) ∥ p(z))$ 계산 가능
{: .prompt-info }



heavy-tailed prior, conditional prior, hierarchical prior 등을 쓰면 multimodal 데이터, 복잡한 구조에서 더 좋은 표현을 얻는다는 결과들도 나오고 있다. 

아무 prior나 말고 정규분포를 쓰는 이유는  
수학적으로 다루기 쉽고, 구현이 단순하면서, latent를 합리적으로 regularize해 주는 **강력한 baseline**이기 때문이지 이게 항상 최적이라서가 아니다.
...
3. [[용어정리] reparameterization trick](https://heygeronimo.tistory.com/40)

4. [multimodal prior 이미지](https://www.researchgate.net/figure/The-visualization-of-two-dimensions-of-a-multimodal-prior-formed-by-results-from-learning_fig1_362429056)

5. [[VAE 논문 리뷰] - Auto-Encoding Variational Bayes](https://kyujinpy.tistory.com/88)

6. [보간 이미지](https://github.com/reshalfahsi/latent-space-vae)
