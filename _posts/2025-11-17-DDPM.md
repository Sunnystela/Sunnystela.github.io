...
KLDivergence로 p*θ(x*{t-1}|x_t)와 forward process의 posteriors(사후확률)를 비교한다.
...
노이즈에서 이미지를 복원하는 역방향 과정을 수행할 신경망의 구조와 노이즈 제거 과정 p*θ에서 사용하는 가우시안 분포의 모수 $\mathbf{\mu}_{\theta}, \mathbf{\Sigma}_{\theta}$를 어떻게 예측하게 할지 선택해야 한다.

선택의 기준을 제시하기 위해 확산 모델과 디노이징 스코어 매칭(Denoising Score Matching)이라는 다른 분야의 모델 사이에 새로운 수학적 연관성을 확립했다.

연결을 통해 단순화된 가중 변분 경계 목표 $\mathbf{L}_{simple}$라는 새로운 손실 함수를 도출했다. 이 손실 함수는 노이즈를 예측하는 오차만을 최소화하도록 구성되어 학습이 매우 효율적이고 안정적이다.
