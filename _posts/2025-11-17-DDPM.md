...
β_T가 충분히 작으면 reverse process와 forward process 가 같은 함수 형태로 everse process의 expressiveness는 p_θ(x_{t−1}|x_t) 에서 가우시안 conditionals를 선택해 부분적으로 보장된다. revese process에서 상태를 예측하는 과정이 수학적으로 잘 정의되고 계산이 간단해지며 모델의 표현력이 어느정도 보장된다.
...
KLDivergence로 p_θ(x_{t-1}|x_t)와 forward process의 posteriors(사후확률)를 비교한다. 
...
q(x_{t−1}|x_t)는 계산하기 어렵지만 q(x_{t−1}|x_t,x_0)는 쉽게 계산할 수 있다. 

> Rao-balckwellized: 기댓값의 variance를 줄여서 추정 정확도를 높이는 수학적 기법. 
